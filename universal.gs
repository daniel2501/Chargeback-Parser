...
var extra_pp_cells = ss.getRange("B16:B18")
...
var extra_pp_formulas = [
  ...
  ['=iferror(REGEXEXTRACT( REGEXEXTRACT($J$3, "Approved by risk (.+?)\)"),"\((.+)@"),"Score Autoreleased")'],
];
...
//
function onEdit(c) {
  if(c.range.rowStart != 30 || c.range.columnStart != 1) {return;};
  setPlatform();
}

function setPlatform()   {
  
  var platform = ss.getRange("A30").getValue(); 
...
['=TEXT(DATEVALUE(regexreplace(REGEXEXTRACT($J$3,"Date:~U~(.+?)~U~"),"Sept", "Sep")),"mm/dd/yyy")'],
